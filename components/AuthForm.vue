<template>
    ...
    <div id="toast-bottom-right" class="fixed flex items-center justify-between w-full max-w-xs p-4 space-x-4 text-gray-500 bg-white divide-x rtl:divide-x-reverse divide-gray-200 rounded-lg shadow right-5 bottom-5 dark:text-gray-400 dark:divide-gray-700 space-x dark:bg-gray-800" role="alert">
    <div class="inline-flex items-center"><div v-if="notifyStatus === 'REGISTERED'" class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200">
    ...
    <div v-if="notifyStatus === 'DUPLICATE EMAIL'" class="ms-3 text-sm font-normal">Email has been registered.</div>
    <div v-if="notifyStatus === 'ERROR'" class="ms-3 text-sm font-normal">Something went wrong.</div>
  </div>
    <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700 right-5" data-dismiss-target="#toast-default" aria-label="Close" @click="notifyStatus=''">
  ...
  type STATUS='REGISTERED'|'DUPLICATE EMAIL'|'ERROR'|''
